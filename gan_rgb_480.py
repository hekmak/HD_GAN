...
class Model():
    ...
    def generator(self, tf_input ,reuse=False ):
        with tf.variable_scope('ham_generator', reuse=reuse):
            print (tf_input)
            #features = normal_conv(tf_input , filters=64, kernel_size=(5,5), padding='same', strides = 1)
            #features = group_norm(features,groups=32)
            #features = tf.nn.relu(features)

            projected = normal_conv(tf_input, filters=1024*20*15, kernel_size=(10,10), padding='same', strides = 10)
            projected = tf.reshape(projected,[self.args.batch_size,20,15,1024])
    ...
    def discriminator(self, tf_input ,reuse=False ):
        with tf.variable_scope('ham_discriminator', reuse=reuse):
            ...
            #pred = tf.compat.v1.nn.dropout(pred,keep_prob=0.5)
    ...
    def compute_loss(self,disc_logits_real,disc_logits_fake):
        ...
        '''
        new_loss3 = new_loss3.assign(tf.to_float(loss))
        Aloss_reduce3 =  Aloss_reduce3.assign( tf.to_float( tf.abs(new_loss3) ))
        '''
    ...
    def tower_loss(self, scope, real_image, gpu_name):
        ...
        #print(disc_logits_real)
        #print(disc_logits_fake)
        #quit()
        #disc_labels = tf.constant([[1,0],[1,0],[0,1],[0,1]],shape=[4,2])
        ##disc_labels = tf.constant([[1],[0]],shape=[2,1],dtype=tf.dtypes.float32)
        #print(disc_logits)
        #print(disc_labels)
        #quit()
            ...
            for i in range(self.args.num_gpus):
                ...
                with tf.device('/gpu:%d' % i):
                    with tf.name_scope('%s_%d' % ('ConfNet', i)) as scope:
                        ...
                        #print (disc_vars)#=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='ham_generator')

                        #print('***********************************&&&^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^')
                        #print (gen_vars)#=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='ham_generator')
                        #quit()
    ...
    def train(self):
        ...
        for e in range(self.args.epoch):
            for i in range(self.args.ds_size//real_batch_size): 
                ...
                '''
                if i%5 == 0:
                    #print("Epoch: {}, Iter: {}, Loss: {:.10f}".format(e, i, loss_depth_value))
                    image_num =  "{:010d}.png".format( int(step/5) )
                    fname_min= os.path.join('./train_preds',image_num)
                    plt.imsave(arr= np.squeeze(to_be_saved_min), fname= fname_min )  
                '''
                if i%50 == 0:
    ...
    def test(self):
        ...
        for j in range( int(100) ):
            #mean = mean+5
            #stddev = stddev+0.5
            #noisak = tf.random.truncated_normal([self.args.batch_size,10,10,1],mean=mean,stddev=stddev)
            # trunc normal in scipy
            noisak = stats.truncnorm.rvs(mean, stddev, size=[self.args.batch_size,10,10,1])
            #noisak = np.random.normal(mean, stddev, size=[self.args.batch_size,10,10,1])
            ...
            for i in range(self.args.batch_size):
                ...
                if self.args.save_test_preds:
                    ...
                    #cv2.imwrite(fname_min,to_be_saved_min)
                    #cv2.imwrite(fname_in,to_be_saved_in)
                    plt.imsave(arr= np.squeeze(to_be_saved_min), fname= fname_min )#, cmap ='nipy_spectral')
                    #plt.imsave(arr= np.log(np.squeeze(to_be_saved_v)), fname= fname_v_vis_log , cmap ='nipy_spectral')
